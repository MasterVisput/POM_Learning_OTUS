...
class AdminProductPage(BasePage):
    ...
    def get_checkbox_by_product_name(self, p_name='Mouse'):
        dashboard_link = self.find_element(DashboardPageSelectors.DASHBOARD_LINK)
        dashboard_link.click()
        if self.is_product_in_tab(p_name):
            products_tab = self.find_elements(DashboardPageSelectors.PRODUCT_TAB)
            for el in products_tab:
                name = el.find_element_by_css_selector('tr td:nth-child(3)')
                if name.text == p_name:
    ...
    def get_edit_button_by_product_name(self, p_name='Mouse'):
        ...
        if self.is_product_in_tab(p_name):
            ...
            for el in products_tab:
                name = el.find_element_by_css_selector('tr td:nth-child(3)')
                if name.text == p_name:
                    edit_button = el.find_element_by_css_selector('tr td:nth-child(8) [data-toggle="tooltip"]')
                    return edit_button
    ...
    def edit_product_from_tab(self, p_name='Mouse', new_p_name='Mouse+21'):
        edit_button = self.get_edit_button_by_product_name(p_name)
        edit_button.click()
